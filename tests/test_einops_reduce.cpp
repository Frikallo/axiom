...
TEST(EinopsReduce, MeanSimple) {
    ...
    axiom::testing::ExpectTensorEquals<float>(result, {2.0f, 5.0f});
}

TEST(EinopsReduce, MaxSimple) {
    ...
    axiom::testing::ExpectTensorEquals<float>(result, {5.0f, 6.0f});
}

TEST(EinopsReduce, MinSimple) {
    ...
    axiom::testing::ExpectTensorEquals<float>(result, {1.0f, 2.0f});
}

TEST(EinopsReduce, MultipleDims) {
    ...
    axiom::testing::ExpectTensorEquals<float>(result, {78.0f, 222.0f});
}

TEST(EinopsReduce, Global) {
    ...
    ASSERT_TRUE(result.size() == 1) << "Should have 1 element";
    axiom::testing::ExpectTensorEquals<float>(result, {10.0f});
}
...
TEST(EinopsReduce, TensorReduceMethod) {
...
TEST(EinopsReduce, Prod) {
    ...
    axiom::testing::ExpectTensorEquals<float>(result, {6.0f, 120.0f});
}

TEST(EinopsReduce, UnityOutput) {
    ...
    ASSERT_TRUE(result.shape() == Shape({2, 3, 1, 1}))
...
TEST(EinopsReduce, EmptyComposition) {
...
TEST(EinopsReduce, PoolingWithAxisSizes) {
    ...
    ASSERT_TRUE(result.shape() == Shape({2, 2, 2, 2}))
        << "Pooling output shape should be (2, 2, 2, 2)";
}

TEST(EinopsReduce, AnonymousAxes) {
...
TEST(EinopsReduce, GlobalPooling) {
...
TEST(EinopsReduce, AdaptivePooling) {
    ...
    // h2 = 6/2 = 3, w2 = 6/2 = 3
    ASSERT_TRUE(result.shape() == Shape({2, 1, 2, 2}))
        << "Adaptive pooling output shape should be (2, 1, 2, 2)";
}

TEST(EinopsReduce, FirstDim) {
    ...
    axiom::testing::ExpectTensorEquals<float>(
        result, {7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f});
}

TEST(EinopsReduce, Any) {
    ...
    ASSERT_TRUE(result.shape() == Shape({2})) << "Shape mismatch";
    ASSERT_TRUE(result.dtype() == DType::Bool) << "Should return Bool tensor";
    const bool *data = result.typed_data<bool>();
    // Row 0: any(false, false, true) = true
    // Row 1: any(false, true, true) = true
    ASSERT_TRUE(data[0] == true) << "Row 0 should be true";
    ASSERT_TRUE(data[1] == true) << "Row 1 should be true";
}

TEST(EinopsReduce, All) {
    ...
    ASSERT_TRUE(data[0] == true) << "Row 0 should be true";
    ASSERT_TRUE(data[1] == false) << "Row 1 should be false";
}

TEST(EinopsReduce, PoolingValues) {
...
TEST(EinopsReduce, RearrangeBasic) {
...
TEST(EinopsReduce, RearrangeFlatten) {
    ...
    ASSERT_TRUE(result.shape() == Shape({6})) << "Shape mismatch";
    axiom::testing::ExpectTensorEquals<float>(
