...
void test_metal_add_int_success() {
    ...
    auto a = axiom::Tensor::full({2, 2}, static_cast<int32_t>(1))
                 .to(axiom::Device::GPU);
    auto b = axiom::Tensor::full({2, 2}, static_cast<int32_t>(2))
                 .to(axiom::Device::GPU);
    ASSERT(a.dtype() == axiom::DType::Int32, "DType mismatch");
    ...
    for (size_t i = 0; i < 4; ++i)
        ASSERT(c_data[i] == 3, "Data mismatch");
#endif
}
...
const std::vector<axiom::DType> all_dtypes = {
    axiom::DType::Bool,    axiom::DType::Int8,    axiom::DType::Int16,
    axiom::DType::Int32,   axiom::DType::Int64,   axiom::DType::UInt8,
    axiom::DType::UInt16,  axiom::DType::UInt32,  axiom::DType::UInt64,
...
void test_arithmetic_op(
    ...
    std::function<axiom::Tensor(const axiom::Tensor &, const axiom::Tensor &)>
        op,
...
void test_all_arithmetic_ops() {
    test_arithmetic_op<float, int, float>("add", axiom::ops::add, 2.5f, 3,
                                          5.5f);
    test_arithmetic_op<int, float, float>("sub", axiom::ops::subtract, 10, 3.5f,
                                          6.5f);
    test_arithmetic_op<double, int, double>("mul", axiom::ops::multiply, 2.5, 4,
                                            10.0);
    test_arithmetic_op<int, int, int>("div", axiom::ops::divide, 10, 3, 3);
    test_arithmetic_op<uint8_t, int8_t, int16_t>("add_mixed_sign",
                                                 axiom::ops::add, 10, -5, 5);
}
