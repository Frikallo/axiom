...
void test_simple_slice() {
    ...
    auto b = a.slice({axiom::Tensor::SliceArg(2, 5)});
...
void test_multi_dim_slice() {
    ...
    auto b = a.slice({
        axiom::Tensor::SliceArg(1, 3),
        axiom::Tensor::SliceArg(2, 4)
    });
...
void test_step_slice() {
    ...
    auto b = a.slice({axiom::Tensor::SliceArg(1, 7, 2)});
...
void test_slice_bounds_error() {
    ...
    ASSERT_THROWS(a.slice({axiom::Tensor::SliceArg(0, 10)}));
    
    // Too many slice arguments
    ASSERT_THROWS(a.slice({axiom::Tensor::SliceArg(0, 1), axiom::Tensor::SliceArg(0, 1), axiom::Tensor::SliceArg(0, 1)}));
}
