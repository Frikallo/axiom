...
bool tensors_equal(const Tensor& cpu, const Tensor& gpu, float tolerance = 1e-5f) {
    auto gpu_cpu = gpu.cpu();
    
    if (cpu.shape() != gpu_cpu.shape()) {
        std::cerr << "Shape mismatch: CPU " << shape_to_string(cpu.shape())
                  << " vs GPU " << shape_to_string(gpu_cpu.shape()) << std::endl;
        return false;
    }
    ...
    case DType::Float32: {
        ...
        for (size_t i = 0; i < cpu.size(); ++i) {
            ...
            if (diff > tolerance && 
                diff > tolerance * std::max(std::abs(cpu_data[i]), std::abs(gpu_data[i]))) {
                std::cerr << "Mismatch at index " << i << ": CPU=" << cpu_data[i]
                          << " GPU=" << gpu_data[i] << " diff=" << diff << std::endl;
                return false;
            }
        }
        break;
    ...
    default:
        std::cerr << "Unsupported dtype for comparison: " << dtype_name(cpu.dtype()) << std::endl;
        return false;
    }
...
void test_subtract_parity() {
    if (!system::is_metal_available()) return;
...
void test_sum_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "Sum full parity failed");
    
    // Axis reduction
    cpu_result = ops::sum(a, {1}, false);
    gpu_result = ops::sum(a.gpu(), {1}, false);
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "Sum axis parity failed");
    
    // Keep dims
    cpu_result = ops::sum(a, {1}, true);
    gpu_result = ops::sum(a.gpu(), {1}, true);
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "Sum keep_dims parity failed");
}
...
void test_matmul_2d_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "MatMul 2D parity failed");
}
...
void test_matmul_batched_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "MatMul batched parity failed");
}
...
void test_where_parity() {
    ...
    auto a = Tensor::randn({4, 5}, DType::Float32, Device::CPU);
    auto b = Tensor::randn({4, 5}, DType::Float32, Device::CPU);
    auto cond = ops::greater(Tensor::randn({4, 5}, DType::Float32, Device::CPU),
...
void test_softmax_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result, 1e-4f), "Softmax parity failed");
}
...
void test_gather_parity() {
    ...
    auto cpu_result = ops::gather(input, 1, indices.unsqueeze(1).expand({4, 1}));
    auto gpu_result = ops::gather(input.gpu(), 1, indices.unsqueeze(1).expand({4, 1}).gpu());
...
void test_broadcast_add_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result), "Broadcast add parity failed");
}
...
void test_broadcast_multiply_parity() {
    ...
    ASSERT(tensors_equal(cpu_result, gpu_result), "Broadcast multiply parity failed");
}
