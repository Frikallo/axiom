...
void test_atan2_basic() {
    ...
    ASSERT(std::abs(data[0] - static_cast<float>(M_PI/4)) < 1e-5f, "atan2(1,1)=pi/4");
    ASSERT(std::abs(data[1] - static_cast<float>(M_PI/2)) < 1e-5f, "atan2(1,0)=pi/2");
...
int main() {
    ...
    std::cout << "=== Logical/Bitwise/Math Operations Tests ===" << std::endl << std::endl;
