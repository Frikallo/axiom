...
template<typename T>
void validate_tensor_data(const Tensor& tensor, const std::vector<T>& expected_data) {
...
void test_memory_order_cpu() {
    ...
    auto c_tensor = zeros({3, 4}, DType::Float32, Device::CPU, MemoryOrder::RowMajor);
    ...
    auto f_tensor = zeros({3, 4}, DType::Float32, Device::CPU, MemoryOrder::ColMajor);
...
void test_memory_order_gpu() {
    ...
    auto c_gpu = zeros({3, 4}, DType::Float32, Device::GPU, MemoryOrder::RowMajor);
    assert(c_gpu.memory_order() == MemoryOrder::RowMajor);
    assert(c_gpu.device() == Device::GPU);
    
    auto f_gpu = zeros({3, 4}, DType::Float32, Device::GPU, MemoryOrder::ColMajor);
...
void test_large_tensors() {
    ...
    auto large_tensor = zeros({large_size, large_size}, DType::Float32, Device::CPU);
...
void test_memory_order_performance() {
    ...
    auto c_matrix = zeros({size, size}, DType::Float32, Device::CPU, MemoryOrder::RowMajor);
    auto f_matrix = zeros({size, size}, DType::Float32, Device::CPU, MemoryOrder::ColMajor);
...
void test_mixed_memory_orders() {
    ...
    auto c_tensor = ones({3, 4}, DType::Float32, Device::CPU, MemoryOrder::RowMajor);
    auto f_tensor = ones({3, 4}, DType::Float32, Device::CPU, MemoryOrder::ColMajor);
...
int main() {
    ...
    std::cout << "GPU Available: " << (is_gpu_available() ? "Yes" : "No") << std::endl;
    std::cout << std::endl;
