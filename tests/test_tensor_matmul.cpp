...
template<typename T>
void assert_tensor_equals_cpu(const axiom::Tensor& t, const std::vector<T>& expected_data, double epsilon = 1e-5) {
    auto t_cpu = t.cpu();
...
void test_matmul_2d_2d() {
    ...
    auto b = axiom::Tensor::arange(12).reshape({3, 4}).astype(axiom::DType::Float32);
    auto c = axiom::ops::matmul(a, b);
    
    ASSERT(c.shape() == axiom::Shape({2, 4}), "Shape mismatch");
    // Expected: [[20, 23, 26, 29], [56, 68, 80, 92]]
    assert_tensor_equals_cpu<float>(c, {20.0f, 23.0f, 26.0f, 29.0f, 56.0f, 68.0f, 80.0f, 92.0f});
}
...
void test_matmul_batched() {
    // (2, 3, 4) @ (2, 4, 5) -> (2, 3, 5)
    auto a = axiom::Tensor::arange(24).reshape({2, 3, 4}).astype(axiom::DType::Float32);
    auto b = axiom::Tensor::arange(40).reshape({2, 4, 5}).astype(axiom::DType::Float32);
    auto c = axiom::ops::matmul(a, b);
...
void test_matmul_broadcast() {
    // (2, 1, 3, 4) @ (4, 5) -> (2, 1, 3, 5)
    auto a = axiom::Tensor::arange(24).reshape({2, 1, 3, 4}).astype(axiom::DType::Float32);
    auto b = axiom::Tensor::arange(20).reshape({4, 5}).astype(axiom::DType::Float32);
    auto c = axiom::ops::matmul(a, b);
...
void test_matmul_broadcast_both() {
    ...
    auto a = axiom::Tensor::arange(24).reshape({2, 1, 3, 4}).astype(axiom::DType::Float32);
    auto b = axiom::Tensor::arange(20).reshape({1, 4, 5}).astype(axiom::DType::Float32);
    auto c = axiom::ops::matmul(a, b);
...
void test_matmul_transpose_a() {
    ...
    auto a = axiom::Tensor::arange(6).reshape({3, 2}).astype(axiom::DType::Float32);
    auto b = axiom::Tensor::arange(12).reshape({3, 4}).astype(axiom::DType::Float32);
...
void test_matmul_transpose_b() {
    ...
    auto a = axiom::Tensor::arange(6).reshape({2, 3}).astype(axiom::DType::Float32);
    auto b = axiom::Tensor::arange(12).reshape({4, 3}).astype(axiom::DType::Float32);
...
void test_matmul_gpu() {
    ...
    auto b = axiom::Tensor::arange(12).reshape({3, 4}).astype(axiom::DType::Float32).to(axiom::Device::GPU);
    auto c = axiom::ops::matmul(a, b);
