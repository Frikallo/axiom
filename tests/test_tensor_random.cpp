...
void test_manual_seed_reproducibility() {
    ...
    for (size_t i = 0; i < 10; ++i) {
        ASSERT(std::abs(data1[i] - data2[i]) < 1e-6, "Seeded random should be reproducible");
    }
}
...
void test_manual_seed_different_seeds() {
    ...
    ASSERT(differences > 90, "Different seeds should produce different results");
}
...
void test_randn_gpu() {
    ...
    auto t = axiom::Tensor::randn({3, 4}, axiom::DType::Float32, axiom::Device::GPU);
...
void test_free_function_manual_seed() {
    ...
    for (size_t i = 0; i < 10; ++i) {
        ASSERT(std::abs(data1[i] - data2[i]) < 1e-6, "Free function manual_seed should work");
    }
}
