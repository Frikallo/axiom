...
void test_scaled_dot_product_attention_cpu() {
    ...
    ASSERT(scores.shape() == Shape({batch, seq_len, seq_len}), "Scores shape wrong");
    
    // Scale
    auto scaled_scores = ops::multiply(scores, Tensor::full({1}, scale, Device::CPU));
    
    // Softmax on last dimension
    auto attn_weights = ops::softmax(scaled_scores, -1);
    ASSERT(attn_weights.shape() == Shape({batch, seq_len, seq_len}), "Attn weights shape wrong");
    ...
    ASSERT(output.shape() == Shape({batch, seq_len, d_k}), "Output shape wrong");
}
...
void test_causal_masked_attention_cpu() {
    ...
    auto causal_mask = Tensor::zeros({seq_len, seq_len}, DType::Bool, Device::CPU);
    ...
    auto masked_scores = ops::masked_fill(scaled_scores.squeeze(0), causal_mask, -1e9f).unsqueeze(0);
...
void test_layer_norm_cpu() {
    ...
    auto x = Tensor::randn({batch, seq_len, hidden}, DType::Float32, Device::CPU);
...
void test_layer_norm_gpu() {
    ...
    auto x = Tensor::randn({batch, seq_len, hidden}, DType::Float32, Device::GPU);
...
void test_dropout_cpu() {
    ...
    ASSERT(kept_ratio > 0.3f && kept_ratio < 0.7f, "Roughly half should be kept");
    
    // Scale factor should be applied: non-zero values should be scaled by 1/(1-p) = 2
    const float* out_data = output.typed_data<float>();
    const bool* mask_data = mask.typed_data<bool>();
    for (size_t i = 0; i < output.size(); ++i) {
        if (mask_data[i]) {
            ASSERT(std::abs(out_data[i] - 2.0f) < 1e-5f, "Kept values should be scaled");
        } else {
...
void test_transformer_block_pattern_cpu() {
    ...
    // Project Q, K, V
    // x_norm: (batch, seq, hidden) @ W: (hidden, hidden) -> (batch, seq, hidden)
    auto Q = ops::matmul(x_norm, Wq);
    auto K = ops::matmul(x_norm, Wk);
    auto V = ops::matmul(x_norm, Wv);
    
    // Reshape for multi-head attention: (batch, seq, num_heads, head_dim) -> (batch, num_heads, seq, head_dim)
    // Note: simplified version, actual implementation would use reshape + transpose
...
int main() {
    ...
    std::cout << "=== Transformer Operations Tests ===" << std::endl << std::endl;
