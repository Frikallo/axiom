...
void test_argmax_keep_dims() {
    ...
    ASSERT(result.shape() == axiom::Shape({2, 1}), "Shape should have kept dim");
}
...
void test_argmin_keep_dims() {
    ...
    ASSERT(result.shape() == axiom::Shape({1, 2}), "Shape should have kept dim");
}
...
void test_argmax_3d() {
    auto t = axiom::Tensor::arange(24).reshape({2, 3, 4}).astype(axiom::DType::Float32);
