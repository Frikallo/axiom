...
void print_tensor_info(const Tensor &tensor, const std::string &name) {
    ...
    std::cout << "  Device: "
              << (tensor.device() == Device::CPU ? "CPU" : "GPU") << "\n";
    ...
    if (tensor.size() <= 12 && tensor.device() == Device::CPU) {
        ...
        if (tensor.dtype() == DType::Float32) {
            ...
            for (size_t i = 0; i < std::min(tensor.size(), size_t(12)); ++i) {
                ...
                if (i < std::min(tensor.size(), size_t(12)) - 1)
                    std::cout << ", ";
            }
...
int main() {
    ...
    try {
        ...
        Tensor c =
            Tensor::full({1}, 5.0f, Device::CPU); // Scalar for broadcasting
        ...
        Tensor float_tensor =
            Tensor::full({2, 3}, 2.5f, Device::CPU); // Float32
        ...
        print_tensor_info(promoted_result,
                          "int_tensor + float_tensor (promoted)");
