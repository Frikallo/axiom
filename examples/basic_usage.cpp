...
int main() {
...
#ifdef __APPLE__
  std::cout << "  Metal GPU: "
            << (Device::GPU == Device::GPU ? "Available" : "Not Available")
            << std::endl;
#else
  ...
  std::cout << "  Element [0,1]: " << data_tensor.item<float>({0, 1})
            << std::endl;
  std::cout << "  Element [1,2]: " << data_tensor.item<float>({1, 2})
            << std::endl;
  ...
  std::cout << "    Base[0,0]: " << base_tensor.item<float>({0, 0})
            << std::endl;
  ...
  std::cout << "  Int32 tensor itemsize: " << int_tensor.itemsize() << " bytes"
            << std::endl;
  std::cout << "  Float64 tensor itemsize: " << float_tensor.itemsize()
            << " bytes" << std::endl;
  std::cout << "  Int32 tensor total bytes: " << int_tensor.nbytes() << " bytes"
            << std::endl;
  ...
  try {
    ...
    std::cout << "  CPU tensor device: "
              << (cpu_tensor.device() == Device::CPU ? "CPU" : "GPU")
              << std::endl;
    std::cout << "  GPU tensor device: "
              << (gpu_tensor.device() == Device::CPU ? "CPU" : "GPU")
              << std::endl;

    // Move back to CPU
    auto back_to_cpu = gpu_tensor.cpu();
    std::cout << "  Moved back to CPU: "
              << (back_to_cpu.device() == Device::CPU ? "CPU" : "GPU")
              << std::endl;
...
#else
  std::cout << "\n7. GPU operations: Not available (Metal requires macOS)"
            << std::endl;
#endif
