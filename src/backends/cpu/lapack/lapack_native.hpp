...
// Native LAPACK backend - throws errors for all operations
// Used as fallback when no LAPACK library is available
...
class NativeLapackBackend : public LapackBackend {
  public:
    NativeLapackBackend() = default;
    ~NativeLapackBackend() override = default;

    // Backend info
    const char *name() const override { return "Native (no LAPACK)"; }
    LapackType type() const override { return LapackType::Native; }
    bool has_lapack() const override { return false; }
  ...
  private:
    [[noreturn]] void throw_no_lapack(const char *op) const;
};
