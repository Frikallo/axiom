...
// Helper to compute log2 for power-of-2 sizes
...
void vfft_c2c_f32(float *data, size_t n, int direction) {
    ...
    // vDSP uses split complex format, so we need to convert
    // Allocate split complex buffers
    std::vector<float> real_part(n), imag_part(n);

    // Deinterleave: [re0, im0, re1, im1, ...] -> separate real/imag arrays
    for (size_t i = 0; i < n; ++i) {
        real_part[i] = data[2 * i];
        imag_part[i] = data[2 * i + 1];
    }
    ...
    FFTDirection fft_dir =
        (direction >= 0) ? kFFTDirection_Forward : kFFTDirection_Inverse;
    vDSP_fft_zip(setup, &split, 1, log2n, fft_dir);

    // Apply scaling for inverse FFT (vDSP doesn't normalize)
    if (direction < 0) {
        float scale = 1.0f / static_cast<float>(n);
        vDSP_vsmul(split.realp, 1, &scale, split.realp, 1, n);
        vDSP_vsmul(split.imagp, 1, &scale, split.imagp, 1, n);
    }

    // Interleave back to output
    for (size_t i = 0; i < n; ++i) {
        data[2 * i] = real_part[i];
        data[2 * i + 1] = imag_part[i];
    }
...
void vfft_c2c_f64(double *data, size_t n, int direction) {
    ...
    // vDSP uses split complex format
    ...
    if (direction < 0) {
        ...
        vDSP_vsmulD(split.realp, 1, &scale, split.realp, 1, n);
        vDSP_vsmulD(split.imagp, 1, &scale, split.imagp, 1, n);
    }
...
void vfft_r2c_f32(const float *input, float *output, size_t n) {
    ...
    // For real FFT, vDSP uses packed format
    // Copy input to split complex (treating real as complex with zero imag)
    std::vector<float> real_part(n), imag_part(n, 0.0f);
    std::copy(input, input + n, real_part.begin());

    DSPSplitComplex split;
    split.realp = real_part.data();
    split.imagp = imag_part.data();

    // Use vDSP_fft_zrip for real-to-complex
    // First, pack the real data
    vDSP_ctoz((DSPComplex *)input, 2, &split, 1, n / 2);

    vDSP_fft_zrip(setup, &split, 1, log2n, kFFTDirection_Forward);

    // Unpack the result to full complex spectrum
    // The output is in special packed format, need to expand
    // DC component
    output[0] = split.realp[0]; // DC real
    output[1] = 0.0f;           // DC imag (always 0 for real input)

    // Nyquist (stored in split.imagp[0] for packed format)
    if (n > 1) {
        output[n] = split.imagp[0]; // Nyquist real
        output[n + 1] = 0.0f;       // Nyquist imag
    }

    // Positive frequencies
    for (size_t i = 1; i < n / 2; ++i) {
        output[2 * i] = split.realp[i];
        output[2 * i + 1] = split.imagp[i];
    }

    // Negative frequencies (conjugate symmetric)
    for (size_t i = 1; i < n / 2; ++i) {
        output[2 * (n - i)] = split.realp[i];
        output[2 * (n - i) + 1] = -split.imagp[i];
    }
...
void vfft_r2c_f64(const double *input, double *output, size_t n) {
    ...
    std::vector<double> real_part(n), imag_part(n, 0.0);
    std::copy(input, input + n, real_part.begin());
    ...
    vDSP_ctozD((DSPDoubleComplex *)input, 2, &split, 1, n / 2);
    vDSP_fft_zripD(setup, &split, 1, log2n, kFFTDirection_Forward);

    // Unpack
    output[0] = split.realp[0];
    output[1] = 0.0;

    if (n > 1) {
        output[n] = split.imagp[0];
        output[n + 1] = 0.0;
    }
...
void vfft_c2r_f32(const float *input, float *output, size_t n) {
    ...
    // Pack complex input into split format for inverse real FFT
    std::vector<float> real_part(n / 2 + 1), imag_part(n / 2 + 1);

    // Pack: DC in realp[0], Nyquist in imagp[0], rest normally
    real_part[0] = input[0];                  // DC
    imag_part[0] = (n > 1) ? input[n] : 0.0f; // Nyquist

    for (size_t i = 1; i < n / 2; ++i) {
        real_part[i] = input[2 * i];
        imag_part[i] = input[2 * i + 1];
    }
    ...
    // Scale (vDSP doesn't normalize inverse)
    float scale = 1.0f / (2.0f * n);
    vDSP_vsmul(split.realp, 1, &scale, split.realp, 1, n / 2);
    vDSP_vsmul(split.imagp, 1, &scale, split.imagp, 1, n / 2);

    // Unpack to real output using vDSP_ztoc
    vDSP_ztoc(&split, 1, (DSPComplex *)output, 2, n / 2);
...
void vfft_c2r_f64(const double *input, double *output, size_t n) {
    ...
    std::vector<double> real_part(n / 2 + 1), imag_part(n / 2 + 1);

    real_part[0] = input[0];
    imag_part[0] = (n > 1) ? input[n] : 0.0;
    ...
    vDSP_vsmulD(split.realp, 1, &scale, split.realp, 1, n / 2);
    vDSP_vsmulD(split.imagp, 1, &scale, split.imagp, 1, n / 2);

    vDSP_ztocD(&split, 1, (DSPDoubleComplex *)output, 2, n / 2);
