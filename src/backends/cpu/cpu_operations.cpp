...
//     // Compute broadcast information
//     auto broadcast_info = ops::compute_broadcast_info(lhs.shape(),
//     rhs.shape());

//     // Determine result type
//     DTypes result_dtype = ops::result_type(lhs, rhs);

//     // Handle comparison and logical operations - they always return bool
//     if (op_type_ == ops::OpType::Equal || op_type_ == ops::OpType::NotEqual
//     ||
//         op_type_ == ops::OpType::Less || op_type_ == ops::OpType::LessEqual
//         || op_type_ == ops::OpType::Greater || op_type_ ==
//         ops::OpType::GreaterEqual || op_type_ == ops::OpType::LogicalAnd ||
//         op_type_ == ops::OpType::LogicalOr ||
//         op_type_ == ops::OpType::LogicalXor) {
//         result_dtype = Bool();
//     }
...
//                           op_type_ == ops::OpType::RightShift);
//     if (is_bitwise_op) {
//         if (std::holds_alternative<Float16>(result_dtype) ||
//             std::holds_alternative<Float32>(result_dtype) ||
//             std::holds_alternative<Float64>(result_dtype) ||
//             std::holds_alternative<Complex64>(result_dtype) ||
//             std::holds_alternative<Complex128>(result_dtype)) {
//             throw TypeError(
//                 "Bitwise operations only support integer types, got " +
//                 dtype_name(result_dtype));
//         }
...
//     case DType::Complex128:
//         // Complex types are handled separately - only arithmetic ops are
//         // allowed The operation registry and operations.cpp should enforce
//         this
//         // before we get here
//         throw TypeError::unsupported_dtype(
//             dtype_name(result_dtype),
...
//             if constexpr (std::is_floating_point_v<T>) {
//                 if constexpr (std::is_same_v<Func, PowerFunc>) {
//                     simd::dispatch_binary_pow(lhs_data, rhs_data,
//                     result_data,
//                                               total_elements);
//                     return;
//                 } else if constexpr (std::is_same_v<Func, Atan2Func>) {
//                     simd::dispatch_binary_atan2(lhs_data, rhs_data,
//                     result_data,
//                                                 total_elements);
//                     return;
//                 } else if constexpr (std::is_same_v<Func, HypotFunc>) {
//                     simd::dispatch_binary_hypot(lhs_data, rhs_data,
//                     result_data,
//                                                 total_elements);
//                     return;
//                 }
...
// void CPUBinaryOperation<Func>::execute_broadcast_loop(
//     const InputT *lhs_data, const InputT *rhs_data, OutputT *result_data,
//     const Shape &lhs_shape, const Shape &rhs_shape, const Shape
//     &result_shape, const Strides &lhs_strides_in, const Strides
//     &rhs_strides_in) const { size_t total_elements =
//     ShapeUtils::size(result_shape); size_t ndim = result_shape.size();
...
//                 lhs_byte_offset);
//             const auto &rhs_val = *reinterpret_cast<const InputT *>(
//                 reinterpret_cast<const uint8_t *>(rhs_data) +
//                 rhs_byte_offset);
...
//         for (size_t j = 0; j < ndim; ++j) {
//             lhs_byte_offset +=
//                 static_cast<int64_t>(result_coords[j]) *
//                 lhs_bcast_strides[j];
//             rhs_byte_offset +=
//                 static_cast<int64_t>(result_coords[j]) *
//                 rhs_bcast_strides[j];
...
//             lhs.shape()) {
//         throw ShapeError(
//             "In-place operation with broadcasting cannot change tensor
//             shape");
//     }
...
// #define DISPATCH_CPU_INPLACE_OP(TYPE_ENUM, TYPE) \
//     case TYPE_ENUM: \
//         execute_inplace_typed<TYPE>(lhs, rhs); \ break;
...
//         for (size_t j = 0; j < lhs_ndim; ++j) {
//             lhs_byte_offset += static_cast<int64_t>(coords[j]) *
//             lhs_strides[j]; rhs_byte_offset +=
//                 static_cast<int64_t>(coords[j]) * rhs_bcast_strides[j];
//         }
...
//     auto broadcast_info = ops::compute_broadcast_info(lhs.shape(),
//     rhs.shape()); DType result_dtype = ops::result_type(lhs, rhs);

//     if (!is_complex(result_dtype)) {
//         throw TypeError(
//             "CPUComplexBinaryOperation requires complex input types");
//     }
...
//     auto broadcast_info = ops::compute_broadcast_info(lhs.shape(),
//     rhs.shape()); const Shape &result_shape = broadcast_info.result_shape;
//     size_t total_elements = ShapeUtils::size(result_shape);
//     size_t ndim = result_shape.size();
...
//     // - isnan/isinf/isfinite always return Bool
//     DType result_dtype = input.dtype();
//     if (is_abs_op && is_complex(input.dtype())) {
//         result_dtype = (input.dtype() == DType::Complex64) ? DType::Float32
//                                                            : DType::Float64;
//     }
//     if (is_logical_not || is_element_test) {
...
//         // operations.cpp except for abs which is already handled above
//         throw TypeError::unsupported_dtype(
//             dtype_name(result_dtype), name() + " (use higher-level
//             dispatch)");
//     default:
//         throw TypeError::unsupported_dtype(dtype_name(result_dtype),
//                                            "CPU unary operations");
...
//         if constexpr (std::is_same_v<T, float>) {
//             if constexpr (std::is_same_v<Func, ExpFunc>) {
//                 accelerate::vexp_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, LogFunc>) {
//                 accelerate::vlog_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SqrtFunc>) {
//                 accelerate::vsqrt_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SinFunc>) {
//                 accelerate::vsin_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CosFunc>) {
//                 accelerate::vcos_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TanFunc>) {
//                 accelerate::vtan_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TanhFunc>) {
//                 accelerate::vtanh_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AbsFunc>) {
//                 accelerate::vabs_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, NegateFunc>) {
//                 accelerate::vneg_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, FloorFunc>) {
//                 accelerate::vfloor_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CeilFunc>) {
//                 accelerate::vceil_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, RoundFunc>) {
//                 accelerate::vround_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TruncFunc>) {
//                 accelerate::vtrunc_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, ReciprocalFunc>) {
//                 accelerate::vrecip_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SquareFunc>) {
//                 accelerate::vsquare_f32(input_data, result_data,
//                                         total_elements);
//                 return;
//             } else if constexpr (std::is_same_v<Func, ErfFunc>) {
//                 accelerate::verf_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CbrtFunc>) {
//                 accelerate::vcbrt_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, ReLUFunc>) {
//                 accelerate::vrelu_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SinhFunc>) {
//                 accelerate::vsinh_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CoshFunc>) {
//                 accelerate::vcosh_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AsinFunc>) {
//                 accelerate::vasin_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AcosFunc>) {
//                 accelerate::vacos_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AtanFunc>) {
//                 accelerate::vatan_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log2Func>) {
//                 accelerate::vlog2_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log10Func>) {
//                 accelerate::vlog10_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log1pFunc>) {
//                 accelerate::vlog1p_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Exp2Func>) {
//                 accelerate::vexp2_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Expm1Func>) {
//                 accelerate::vexpm1_f32(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, RsqrtFunc>) {
//                 accelerate::vrsqrt_f32(input_data, result_data,
//                 total_elements); return;
//             }
//         }
//         if constexpr (std::is_same_v<T, double>) {
//             if constexpr (std::is_same_v<Func, ExpFunc>) {
//                 accelerate::vexp_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, LogFunc>) {
//                 accelerate::vlog_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SqrtFunc>) {
//                 accelerate::vsqrt_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SinFunc>) {
//                 accelerate::vsin_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CosFunc>) {
//                 accelerate::vcos_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TanFunc>) {
//                 accelerate::vtan_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TanhFunc>) {
//                 accelerate::vtanh_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AbsFunc>) {
//                 accelerate::vabs_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, NegateFunc>) {
//                 accelerate::vneg_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, FloorFunc>) {
//                 accelerate::vfloor_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CeilFunc>) {
//                 accelerate::vceil_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, RoundFunc>) {
//                 accelerate::vround_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, TruncFunc>) {
//                 accelerate::vtrunc_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, ReciprocalFunc>) {
//                 accelerate::vrecip_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SquareFunc>) {
//                 accelerate::vsquare_f64(input_data, result_data,
//                                         total_elements);
//                 return;
//             } else if constexpr (std::is_same_v<Func, ErfFunc>) {
//                 accelerate::verf_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CbrtFunc>) {
//                 accelerate::vcbrt_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, ReLUFunc>) {
//                 accelerate::vrelu_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, SinhFunc>) {
//                 accelerate::vsinh_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, CoshFunc>) {
//                 accelerate::vcosh_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AsinFunc>) {
//                 accelerate::vasin_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AcosFunc>) {
//                 accelerate::vacos_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, AtanFunc>) {
//                 accelerate::vatan_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log2Func>) {
//                 accelerate::vlog2_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log10Func>) {
//                 accelerate::vlog10_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Log1pFunc>) {
//                 accelerate::vlog1p_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Exp2Func>) {
//                 accelerate::vexp2_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, Expm1Func>) {
//                 accelerate::vexpm1_f64(input_data, result_data,
//                 total_elements); return;
//             } else if constexpr (std::is_same_v<Func, RsqrtFunc>) {
//                 accelerate::vrsqrt_f64(input_data, result_data,
//                 total_elements); return;
//             }
//         }
//     }
...
//                 // Activation functions
//                 if constexpr (std::is_same_v<Func, SigmoidFunc>) {
//                     simd::dispatch_activation_sigmoid(input_data,
//                     result_data,
//                                                       total_elements);
//                     return;
//                 } else if constexpr (std::is_same_v<Func, SiLUFunc>) {
...
// #ifdef AXIOM_USE_OPENMP
//     if (input.is_contiguous() &&
//     parallel::should_parallelize(total_elements)) {
//         ptrdiff_t n = static_cast<ptrdiff_t>(total_elements);
// #pragma omp parallel for schedule(static)
//         for (ptrdiff_t i = 0; i < n; ++i) {
//             result_data[i] = func_(input_data[i]);
...
//     switch (input.dtype()) {
//     case DType::Float16:
//         // Use Float32 accumulation for numerical stability, convert result
//         back return execute_reduction_typed<float16_t>(input, axis,
//         keep_dims);
//     case DType::Float32:
//         return execute_reduction_typed<float>(input, axis, keep_dims);
//     case DType::Float64:
...
//             Shape scalar_shape = keep_dims ? Shape(input.ndim(), 1) :
//             Shape{1}; Tensor result(scalar_shape, accum_dtype, Device::CPU);
//             result.template typed_data<float>()[0] = result_val;
...
// template <typename T>
// void CPUMatMulOperation::matmul_2d(const T *a_data, const T *b_data, T
// *c_data,
//                                    size_t M, size_t N, size_t K,
//                                    size_t a_row_stride, size_t a_col_stride,
//                                    size_t b_row_stride, size_t b_col_stride,
...
//         bool b_row_major = (b_col_stride == 1);
//         bool b_col_major = (b_row_stride == 1);
//         bool c_row_major = (c_col_stride == 1) || (c_col_stride == 0 && N ==
//         1);
...
//             // Determine transpose flags and leading dimensions for BLAS
//             bool trans_a =
//                 !a_row_major; // If A is col-major, it looks transposed to
//                 BLAS
//             bool trans_b = !b_row_major;

//             // Leading dimensions (stride of the non-unit dimension)
//             // For row-major: lda = stride of row (number of columns in
//             memory)
//             // For col-major: lda = stride of column (number of rows in
//             memory) size_t lda = a_row_major ? (a_row_stride > 0 ?
//             a_row_stride : K)
//                                      : (a_col_stride > 0 ? a_col_stride : M);
//             size_t ldb = b_row_major ? (b_row_stride > 0 ? b_row_stride : N)
//                                      : (b_col_stride > 0 ? b_col_stride : K);
...
//                 if constexpr (std::is_same_v<T, float>) {
//                     backend.sgemm(trans_a, trans_b, M, N, K, 1.0f, a_data,
//                     lda,
//                                   b_data, ldb, 0.0f, c_data, ldc);
//                 } else {
//                     backend.dgemm(trans_a, trans_b, M, N, K, 1.0, a_data,
//                     lda,
//                                   b_data, ldb, 0.0, c_data, ldc);
//                 }
//                 return;
...
//     // Fallback: Cache-blocked matrix multiplication with proper memory
//     access
//     // Key optimization: accumulate C in a local tile buffer to avoid
//     repeated
//     // loads/stores to C on every K iteration (which causes memory bandwidth
//     // bottleneck)
//     constexpr size_t TILE_M = 64;
...
//     // Scalar result (rank-0 tensor) is handled naturally with empty shape
//     // Use empty() instead of zeros() - BLAS will write all values, and
//     fallback
//     // path zeros explicitly. This avoids unnecessary memory initialization.
//     Tensor result = Tensor::empty(result_shape, a.dtype(), Device::CPU);
...
//         // c_col=0 We need to determine which based on input shapes
//         if (a_ndim == 1 && b_ndim >= 2) {
//             // (K,) @ (..., K, N) -> (..., N) - result is conceptually a row,
//             so
//             // col varies
//             c_row_stride = 0;
//             c_col_stride = result.strides()[0] / c_itemsize;
//         } else {
//             // (..., M, K) @ (K,) -> (..., M) - result is conceptually a
//             column,
//             // so row varies
//             c_row_stride = result.strides()[0] / c_itemsize;
//             c_col_stride = 0;
...
//         } else {
//             backend.dgemm(transpose_a, transpose_b, M, N, K_a, 1.0,
//                           a.typed_data<double>(), lda,
//                           b.typed_data<double>(), ldb, 0.0,
//                           result.typed_data<double>(), ldc);
//         }
//         return result;
//     }
...
// template <typename T>
// Tensor CPUArgMaxOperation::execute_argmax_typed(const Tensor &input, int
// axis,
//                                                 bool keep_dims) const {
//     size_t ndim = input.ndim();
...
// template <typename T>
// Tensor CPUArgMinOperation::execute_argmin_typed(const Tensor &input, int
// axis,
//                                                 bool keep_dims) const {
//     size_t ndim = input.ndim();
...
// // Helper to compute broadcast strides: maps input shape to output shape
// strides
// // Returns strides in bytes where broadcasted dimensions have stride 0
// static Strides compute_broadcast_strides(const Shape &input_shape,
//                                          const Shape &output_shape,
...
// // Helper to get value from tensor at byte offset, converting to output type
// T template <typename T> static T get_tensor_value_at(const void *data, size_t
// byte_offset,
//                              DType dtype) {
//     switch (dtype) {
//     case DType::Bool:
...
//     // Get data pointers
//     const uint8_t *cond_data = static_cast<const uint8_t
//     *>(condition.data()); const void *a_data = a.data(); const void *b_data =
//     b.data(); T *result_data = result.typed_data<T>();
...
//             T value = cond_val
//                           ? get_tensor_value_at<T>(a_data, a_offset, a_dtype)
//                           : get_tensor_value_at<T>(b_data, b_offset,
//                           b_dtype);
//             result_data[result_offset / result_itemsize] = value;
//         }
//         return result;
...
//         bool cond_val = false;
//         if (cond_dtype == DType::Bool) {
//             cond_val = *reinterpret_cast<const bool *>(cond_data +
//             cond_offset);
//         } else {
//             // For numeric types, non-zero is true
//             // Check if any byte is non-zero
...
//         // Select from a or b based on condition, with proper type conversion
//         T value = cond_val ? get_tensor_value_at<T>(a_data, a_offset,
//         a_dtype)
//                            : get_tensor_value_at<T>(b_data, b_offset,
//                            b_dtype);
//         result_data[result_offset / result_itemsize] = value;
...
//         DISPATCH_WHERE(DType::Bool, bool)
//     default:
//         throw TypeError::unsupported_dtype(dtype_name(output_dtype),
//         "Where");
//     }
// #undef DISPATCH_WHERE
// }
...
//         for (ptrdiff_t i = 0; i < n; ++i) {
//             auto coords =
//                 ShapeUtils::unravel_index(static_cast<size_t>(i),
//                 result_shape);
//             size_t mask_offset = ShapeUtils::linear_index(coords,
//             mask_strides); bool mask_val =
//                 *reinterpret_cast<const uint8_t *>(
//                     static_cast<const uint8_t *>(mask_ptr) + mask_offset) !=
//                     0;
//             size_t input_offset =
//                 ShapeUtils::linear_index(coords, input_strides);
//             T input_val = *reinterpret_cast<const T *>(
...
//         // Get input value
//         size_t input_offset = ShapeUtils::linear_index(coords,
//         input_strides); T input_val = *reinterpret_cast<const T *>(
//             static_cast<const uint8_t *>(input_ptr) + input_offset);
...
//                             *>(mask_expanded.data()) + mask_offset) != 0;
//         if (mask_val) {
//             count++;
//         }
//     }
...
//         // Get the value from input
//         size_t input_offset = ShapeUtils::linear_index(coords,
//         input_strides); result_data[i] = *reinterpret_cast<const T *>(
//             static_cast<const uint8_t *>(input_ptr) + input_offset);
//     }
...
// template <typename T>
// Tensor CPUScatterOperation::execute_scatter_typed(const Tensor &input, int
// dim,
//                                                   const Tensor &indices,
//                                                   const Tensor &src) const {
//     // Normalize dim
...
// #ifdef AXIOM_USE_ACCELERATE
//     // Use Accelerate for contiguous softmax along the last axis (common
//     case) if (input.is_contiguous() && inner_size == 1) {
//         for (size_t outer = 0; outer < outer_size; ++outer) {
//             const T *slice_in = input_data + outer * axis_size;
//             T *slice_out = result_data + outer * axis_size;
...
//                 T log_sum = std::log(sum_exp);
//                 for (size_t k = 0; k < axis_size; ++k) {
//                     size_t idx = (outer * axis_size + k) * inner_size +
//                     inner; result_data[idx] = input_data[idx] - max_val -
//                     log_sum;
//                 }
//             } else {
//                 for (size_t k = 0; k < axis_size; ++k) {
//                     size_t idx = (outer * axis_size + k) * inner_size +
//                     inner; result_data[idx] /= sum_exp;
//                 }
//             }
//         }
//         return result;
...
//     switch (input.dtype()) {
//     case DType::Float16: {
//         // For float16, compute in float32 for numerical stability, then
//         convert
//         // back
//         Tensor input_f32 = input.astype(DType::Float32);
//         Tensor result_f32 = execute_softmax_typed<float>(input_f32, ax);
...
//     OperationRegistry::register_operation(
//         OpType::Power, Device::CPU,
//         std::make_unique<CPUBinaryOperation<PowerFunc>>(OpType::Power,
//         "power",
//                                                         PowerFunc{}));
...
//     OperationRegistry::register_operation(
//         OpType::Equal, Device::CPU,
//         std::make_unique<CPUBinaryOperation<EqualFunc>>(OpType::Equal,
//         "equal",
//                                                         EqualFunc{}));
...
//     OperationRegistry::register_operation(
//         OpType::Atan2, Device::CPU,
//         std::make_unique<CPUBinaryOperation<Atan2Func>>(OpType::Atan2,
//         "atan2",
//                                                         Atan2Func{}));

//     OperationRegistry::register_operation(
//         OpType::Hypot, Device::CPU,
//         std::make_unique<CPUBinaryOperation<HypotFunc>>(OpType::Hypot,
//         "hypot",
//                                                         HypotFunc{}));
...
//     OperationRegistry::register_operation(
//         OpType::Floor, Device::CPU,
//         std::make_unique<CPUUnaryOperation<FloorFunc>>(OpType::Floor,
//         "floor",
//                                                        FloorFunc{}));
//     OperationRegistry::register_operation(
//         OpType::Ceil, Device::CPU,
...
//     OperationRegistry::register_operation(
//         OpType::Trunc, Device::CPU,
//         std::make_unique<CPUUnaryOperation<TruncFunc>>(OpType::Trunc,
//         "trunc",
//                                                        TruncFunc{}));
//     OperationRegistry::register_operation(
//         OpType::Round, Device::CPU,
//         std::make_unique<CPUUnaryOperation<RoundFunc>>(OpType::Round,
//         "round",
//                                                        RoundFunc{}));
//     OperationRegistry::register_operation(
//         OpType::Reciprocal, Device::CPU,
...
//     OperationRegistry::register_operation(
//         OpType::IsNaN, Device::CPU,
//         std::make_unique<CPUUnaryOperation<IsNaNFunc>>(OpType::IsNaN,
//         "isnan",
//                                                        IsNaNFunc{}));
//     OperationRegistry::register_operation(
//         OpType::IsInf, Device::CPU,
//         std::make_unique<CPUUnaryOperation<IsInfFunc>>(OpType::IsInf,
//         "isinf",
//                                                        IsInfFunc{}));
//     OperationRegistry::register_operation(
//         OpType::IsFinite, Device::CPU,
...
//     OperationRegistry::register_operation(
//         OpType::Mean, Device::CPU,
//         std::make_unique<CPUReductionOperation<SumFunc>>(OpType::Mean,
//         "mean",
//                                                          SumFunc{}));
//     OperationRegistry::register_operation(
//         OpType::Max, Device::CPU,
...
//     OperationRegistry::register_operation(
//         OpType::Prod, Device::CPU,
//         std::make_unique<CPUReductionOperation<ProdFunc>>(OpType::Prod,
//         "prod",
//                                                           ProdFunc{}));
...
//         OpType::Gather, Device::CPU, std::make_unique<CPUGatherOperation>());
//     OperationRegistry::register_operation(
//         OpType::Scatter, Device::CPU,
//         std::make_unique<CPUScatterOperation>());
//     OperationRegistry::register_operation(
//         OpType::IndexSelect, Device::CPU,
//         std::make_unique<CPUIndexSelectOperation>());
