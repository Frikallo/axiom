...
std::string Tracer::dump() const {
  ...
  for (const auto& event : events_) {
    ...
    oss << std::left << std::setw(20) << event.op_name
        << std::setw(15) << std::fixed << std::setprecision(2) << duration_us
    ...
    if (event.materialized) materialized_count++;
  }
...
ScopedTrace::~ScopedTrace() {
  if (Tracer::instance().is_enabled()) {
    ...
    Tracer::instance().record(op_name_, desc_, duration, memory_bytes_, materialized_);
  }
}
