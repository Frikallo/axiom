...
// static void assert_complex_legal(OpType op, DTypes dtype) {
//     if (is_complex(dtype) &&
...
// DTypes promote_types(DTypes lhs, DTypes rhs) {
//     if (lhs.index() == rhs.index())
//         return lhs;

//     // Highest rank wins
//     static const std::map<DTypes, int> rank = {
//         {Bool(), 0},       {Int8(), 1},       {UInt8(), 2},    {Int16(), 3},
//         {UInt16(), 4},     {Int32(), 5},      {UInt32(), 6},   {Int64(), 7},
//         {UInt64(), 8},     {Float16(), 9},    {Float32(), 10}, {Float64(),
//         11}, {Complex64(), 12}, {Complex128(), 13}};

//     DTypes t1 = rank.at(lhs) > rank.at(rhs) ? lhs : rhs;
//     DTypes t2 = rank.at(lhs) > rank.at(rhs) ? rhs : lhs;
...
//     // Both are integers
//     bool signed1 = is_signed(t1);
//     bool signed2 = is_signed(t2);
...
// DTypes result_type(const Tensor &lhs, const Tensor &rhs) {
...
//     (void)axis;
//     (void)keep_dims;
//     throw RuntimeError::not_implemented("Reduction operations for " +
//     name());
// }
...
// Tensor Operation::execute_masked_fill(const Tensor &input, const Tensor
// &mask,
//                                       const Tensor &value) const {
//     (void)input;
//     (void)mask;
//     (void)value;
//     throw RuntimeError::not_implemented("MaskedFill operations for " +
//     name());
// }
...
//     (void)dim;
//     (void)indices;
//     throw RuntimeError::not_implemented("IndexSelect operations for " +
//     name());
// }

// Tensor Operation::execute_cast(const Tensor &input, DTypes target_dtype)
// const {
//     (void)input;
//     (void)target_dtype;
...
// //
// ============================================================================
// static Tensor execute_reduction_operation(OpType op_type, const Tensor
// &input,
//                                           const std::vector<int> &axis,
//                                           bool keep_dims) {
//     // Handle complex types directly
...
//     const auto *op = OperationRegistry::get_operation(op_type,
//     target_device); if (!op) {
...
//     Tensor a_target = (a.device() == target_device) ? a :
//     a.to(target_device); Tensor b_target = (b.device() == target_device) ? b
//     : b.to(target_device);
...
//             std::vector<size_t> coords(broadcast_info.result_shape.size(),
//             0); for (size_t i = 0; i < total; ++i) {
...
//     // Fallback to CPU if GPU op is not available or doesn't support the
//     inputs if (target_device == Device::GPU &&
...
//     auto inv_scale_tensor = Tensor::full({1}, static_cast<float>(1.0 /
//     scale)); return multiply(rounded, inv_scale_tensor);
// }
...
// Tensor clip(const Tensor &input, const Tensor &min_val, const Tensor
// &max_val) {
...
// Tensor mean(const Tensor &input, const std::vector<int> &axis, bool
// keep_dims) {
...
// Tensor prod(const Tensor &input, const std::vector<int> &axis, bool
// keep_dims) {
...
//     Tensor mask_on_device = (mask.device() == device) ? mask :
//     mask.to(device); Tensor value_on_device =
...
// Tensor layer_norm(const Tensor &input, const Tensor &weight, const Tensor
// &bias,
//                   int axis, float eps) {
//     // Determine device
//     Device device = Device::CPU;
...
//     Tensor bias_on_device = (bias.device() == device) ? bias :
//     bias.to(device);
...
// // Dropout operation
// std::pair<Tensor, Tensor> dropout(const Tensor &input, float p, bool
// training) {
//     if (!training || p == 0.0f) {
