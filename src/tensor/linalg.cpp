...
// // Check that tensor is on CPU and make contiguous column-major copy if
// needed
// // LAPACK expects column-major (Fortran) order
// Tensor ensure_cpu_contiguous_colmajor(const Tensor &t) {
//     auto cpu_t = t.device() == Device::CPU ? t : t.cpu();
...
//     }
//     if (info > 0) {
//         throw RuntimeError("LAPACK " + routine + ": computation failed
//         (info=" +
//                            std::to_string(info) + ")");
//     }
// }
...
//             int info = backend.sgetrf(static_cast<int>(n),
//             static_cast<int>(n),
//                                       a_col.data(), lda, ipiv.data());
//             if (info > 0) {
//                 // Singular matrix, det = 0
...
//             int info = backend.dgetrf(static_cast<int>(n),
//             static_cast<int>(n),
//                                       a_col.data(), lda, ipiv.data());
//             if (info > 0) {
//                 result_data[b] = 0.0;
...
//             int info = backend.cgetrf(static_cast<int>(n),
//             static_cast<int>(n),
//                                       a_col.data(), lda, ipiv.data());
//             if (info > 0) {
//                 result_data[b] = complex64_t(0.0f, 0.0f);
...
//             int info = backend.zgetrf(static_cast<int>(n),
//             static_cast<int>(n),
//                                       a_col.data(), lda, ipiv.data());
//             if (info > 0) {
//                 result_data[b] = complex128_t(0.0, 0.0);
...
//             int info = backend.sgesv(static_cast<int>(n),
//                                      static_cast<int>(nrhs), a_col.data(),
//                                      lda, ipiv.data(), b_col.data(), ldb);
//             check_lapack_info(info, "sgesv");
...
//         int info = backend.sgesdd(
//             jobz, static_cast<int>(m), static_cast<int>(n), dummy_a.data(),
//             lda, dummy_s.data(), dummy_u.data(), ldu, dummy_vt.data(), ldvt,
//             &work_query, -1, iwork.data());
//         check_lapack_info(info, "sgesdd workspace query");
...
//         result.U = Tensor(u_shape, DType::Complex64);
//         result.S = Tensor(s_shape, DType::Float32); // Singular values are
//         real result.Vh = Tensor(vt_shape, DType::Complex64);
...
//         result.U = Tensor(u_shape, DType::Complex128);
//         result.S = Tensor(s_shape, DType::Float64); // Singular values are
//         real result.Vh = Tensor(vt_shape, DType::Complex128);
...
//             int info =
//                 backend.dgeev('N', 'V', static_cast<int>(n), a_col.data(),
//                 lda,
//                               wr.data(), wi.data(), nullptr, 1, vr.data(),
//                               ldvr, work.data(),
//                               static_cast<int>(work.size()));
//             check_lapack_info(info, "dgeev");
...
//             // Convert eigenvectors to complex (LAPACK stores complex
//             conjugate
//             // pairs)
//             for (size_t j = 0; j < n;) {
//                 if (wi[j] == 0.0) {
...
//                             complex128_t(vr[j * n + i], vr[(j + 1) * n + i]);
//                         eigenvec_batch[i * n + j + 1] =
//                             complex128_t(vr[j * n + i], -vr[(j + 1) * n +
//                             i]);
//                     }
//                     j += 2;
//                 }
...
//         }
//     } else {
//         return lstsq(a.astype(DType::Float64), b.astype(DType::Float64),
//         rcond);
//     }
...
//         float thresh_f = static_cast<float>(threshold);
//         for (size_t i = 0; i < S.size(); ++i) {
//             s_inv_data[i] = (s_data[i] > thresh_f) ? (1.0f / s_data[i]) :
//             0.0f;
//         }
//     } else {
//         double *s_data = S.typed_data<double>();
//         double *s_inv_data = S_inv.typed_data<double>();
//         for (size_t i = 0; i < S.size(); ++i) {
//             s_inv_data[i] = (s_data[i] > threshold) ? (1.0 / s_data[i]) :
//             0.0;
//         }
//     }
...
//             for (size_t i = 0; i < n; ++i) {
//                 for (size_t j = 0; j < k; ++j) {
//                     v_scaled[i * k + j] = vht_batch[i * k + j] *
//                     s_inv_batch[j];
//                 }
//             }
...
//     if (tol < 0) {
//         size_t k = std::min(a.shape()[a.ndim() - 2], a.shape()[a.ndim() -
//         1]); tol =
...
//                                         b_work.typed_data<float>(), 1);
//             return Tensor::full({}, result);
//         } else if (a.dtype() == DType::Float64 && b.dtype() ==
//         DType::Float64) {
//             auto &backend = backends::cpu::blas::get_blas_backend();
//             double result = backend.ddot(n, a_work.typed_data<double>(), 1,
//                                          b_work.typed_data<double>(), 1);
...
//             } else {
//                 // Convert to float64 for other types
//                 return dot(a.astype(DType::Float64),
//                 b.astype(DType::Float64));
//             }
//         }
//     }
...
//     // Matrix-vector product: (..., M, N) @ (..., N) -> (..., M)
//     if (x1.ndim() < 2) {
//         throw ShapeError("matvec: first argument must be at least 2D
//         matrix");
//     }
//     if (x2.ndim() < 1) {
//         throw ShapeError("matvec: second argument must be at least 1D
//         vector");
//     }
...
//             // BLAS gemv with row-major: y = A * x
//             // A is (m, n), x is (n,), y is (m,)
//             // sgemv(trans=false, M=m, N=n, alpha, A, lda=n, x, incx, beta,
//             y,
//             // incy)
//             backend.sgemv(false, m, n, 1.0f, mat_batch, n, vec_batch, 1,
//             0.0f,
//                           result_batch, 1);
//         }
//     } else if (promoted_dtype == DType::Float64) {
...
//             for (size_t b = 0; b < batch_size; ++b) {
//                 const cplx *mat_batch = mat_data + b * m * n;
//                 const cplx *vec_batch = vec_data + (x2.ndim() > 1 ? b * n :
//                 0); cplx *result_batch = result_data + b * m;
...
//             }
//         } else {
//             return matvec(x1.astype(DType::Float64),
//             x2.astype(DType::Float64));
//         }
//     }
...
//     // Vector-matrix product: (..., M) @ (..., M, N) -> (..., N)
//     if (x1.ndim() < 1) {
//         throw ShapeError("vecmat: first argument must be at least 1D
//         vector");
//     }
//     if (x2.ndim() < 2) {
//         throw ShapeError("vecmat: second argument must be at least 2D
//         matrix");
//     }
...
//             // vecmat: v @ A where v is (m,), A is (m,n) -> (n,)
//             // This equals A^T @ v for column vector interpretation
//             // BLAS row-major: sgemv(trans=true, M=m, N=n, ...) computes y =
//             A^T
//             // @ x
//             backend.sgemv(true, m, n, 1.0f, mat_batch, n, vec_batch, 1, 0.0f,
//                           result_batch, 1);
...
//             for (size_t b = 0; b < batch_size; ++b) {
//                 const cplx *vec_batch = vec_data + (x1.ndim() > 1 ? b * m :
//                 0); const cplx *mat_batch = mat_data + b * m * n; cplx
//                 *result_batch = result_data + b * n;
...
//             }
//         } else {
//             return vecmat(x1.astype(DType::Float64),
//             x2.astype(DType::Float64));
//         }
//     }
