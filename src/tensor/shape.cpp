...
Strides ShapeUtils::get_contiguous_strides(const Shape &shape, size_t itemsize,
                                           MemoryOrder order) {
...
size_t ShapeUtils::linear_index(const std::vector<size_t> &indices,
    ...
    if (indices.size() != strides.size()) {
        throw ShapeError("Number of indices (" +
...
Shape squeeze_shape(const Shape &shape, int axis) {
    ...
    if (axis == -1) {
        ...
        if (shape[axis] != 1) {
            throw ShapeError("Cannot squeeze dimension " +
                             std::to_string(axis) + " that has size " +
                             std::to_string(shape[axis]) + " (not 1)");
        }
...
Shape unsqueeze_shape(const Shape &shape, int axis) {
    ...
    if (axis < 0)
        axis += shape.size() + 1;
    if (axis < 0 || axis > static_cast<int>(shape.size())) {
        throw ShapeError("Axis " + std::to_string(axis) +
                         " out of bounds for unsqueeze (shape has " +
...
Shape reshape_shape(const Shape &current_shape, const Shape &new_shape) {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
        if (static_cast<int>(result[i]) == -1) {
            if (infer_idx != -1) {
                throw ShapeError(
                    "Only one dimension can be inferred in reshape");
            }
            infer_idx = i;
        }
    }
