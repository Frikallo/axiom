...
uint32_t dtype_to_uint32(DType dtype) {
    return static_cast<uint32_t>(dtype);
}

// Convert uint32_t back to DType
DType uint32_to_dtype(uint32_t value) {
...
void validate_header(const AxiomFileHeader& header) {
    ...
    if (header.version > AXIOM_FILE_VERSION) {
        throw FileFormatError("Unsupported file version: " + std::to_string(header.version));
    }
    
    if (header.ndim > 32) { // Reasonable limit
        throw FileFormatError("Too many dimensions: " + std::to_string(header.ndim));
    }
...
void save_archive(const std::map<std::string, Tensor>& tensors,
    ...
    std::ofstream file(filename, std::ios::binary);
    if (!file.is_open()) {
        throw SerializationError("Cannot open archive file for writing: " + filename);
    }
    ...
    file.write(reinterpret_cast<const char*>(&archive_header), sizeof(archive_header));
    ...
    for (const auto& [name, tensor] : tensors) {
        ...
        size_t tensor_size = static_cast<size_t>(end_pos - tensor_positions[entry_idx]);
        
        // Fill index entry
        std::memset(&index_entries[entry_idx], 0, sizeof(TensorIndexEntry));
        std::strncpy(index_entries[entry_idx].name, name.c_str(), 63);
        index_entries[entry_idx].offset = static_cast<uint64_t>(tensor_positions[entry_idx]);
...
std::map<std::string, Tensor> load_archive(const std::string& filename, 
    ...
    std::ifstream file(filename, std::ios::binary);
    if (!file.is_open()) {
        throw SerializationError("Cannot open archive file for reading: " + filename);
    }
