...
void save_archive(const std::map<std::string, Tensor> &tensors,
    ...
    for (const auto &[name, tensor] : tensors) {
        if (name.length() >= 63) {
            throw SerializationError("Tensor name too long (max 63 chars): " +
                                     name);
        }
...
std::map<std::string, Tensor> load_archive(const std::string &filename,
    ...
    // Read archive header
    ArchiveHeader archive_header;
    file.read(reinterpret_cast<char *>(&archive_header),
              sizeof(archive_header));
...
std::string to_string(const Tensor &tensor) {
    ...
    // Add tensor.repr() to the string (use original tensor, not CPU copy)
    ss << " " << tensor.repr();
