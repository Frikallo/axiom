...
void write_file(const std::string &filename, const uint8_t *data, size_t size) {
...
build_tensor_data(::flatbuffers::FlatBufferBuilder &builder,
    ...
    Tensor cpu_tensor = tensor.device() == Device::CPU ? tensor : tensor.cpu();
    if (!cpu_tensor.is_c_contiguous() && !cpu_tensor.is_f_contiguous()) {
    ...
    auto data_offset = builder.CreateVector(data_ptr, cpu_tensor.nbytes());
