//=============================================================================
// src/tensor/type_conversion.hpp - Type conversion utilities (FIXED)
//=============================================================================
...
template <> struct dtype_to_type<DType::Int8> { using type = int8_t; };
template <> struct dtype_to_type<DType::Int16> { using type = int16_t; };
...
template <> struct dtype_to_type<DType::Float16> { using type = float16_t; };
...
template <> struct dtype_to_type<DType::Complex64> { using type = complex64_t; };
template <> struct dtype_to_type<DType::Complex128> { using type = complex128_t; };
...
struct numeric_converter {
    static To convert(const From& value) {
        ...
        else if constexpr (std::is_same_v<From, complex64_t> || std::is_same_v<From, complex128_t>) {
            if constexpr (std::is_same_v<To, complex64_t> || std::is_same_v<To, complex128_t>) {
        ...
        // Real to complex
        else if constexpr (std::is_same_v<To, complex64_t> || std::is_same_v<To, complex128_t>) {
        ...
        // Integer to integer with range checking
            ...
            if constexpr (sizeof(To) >= sizeof(From) && 
                ...
                if (value < min_val || value > max_val) {
                    // NumPy behavior: wrap around for integer overflow
                    return static_cast<To>(value);
                }
                return static_cast<To>(value);
            }
        ...
        // Integer to float
                          ...
                          (std::is_floating_point_v<To> || std::is_same_v<To, float16_t>)) {
            return static_cast<To>(value);
        }
        // Float to integer (truncate)
        else if constexpr ((std::is_floating_point_v<From> || std::is_same_v<From, float16_t>) && 
...
void convert_dtype(void* dst, const void* src, size_t count, 
...
void convert_dtype_strided(void* dst, const void* src, 
                          const Shape& shape, const Strides& dst_strides, const Strides& src_strides,
                          DType dst_dtype, DType src_dtype, size_t dst_offset = 0, size_t src_offset = 0) {
    ...
    for (size_t i = 0; i < total_elements; ++i) {
        ...
        size_t src_linear_offset = src_offset + ShapeUtils::linear_index(indices, src_strides);
        size_t dst_linear_offset = dst_offset + ShapeUtils::linear_index(indices, dst_strides);
        
        const void* src_element = static_cast<const uint8_t*>(src) + src_linear_offset;
...
// ============================================================================
// Utility functions for dtype queries
// ============================================================================
