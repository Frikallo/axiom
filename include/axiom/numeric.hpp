...
template <typename T>
constexpr T nan() {
  static_assert(std::is_floating_point_v<T>, "nan<T> requires floating-point type");
...
template <typename T>
constexpr T inf() {
  static_assert(std::is_floating_point_v<T>, "inf<T> requires floating-point type");
...
template <typename T>
constexpr T neg_inf() {
  static_assert(std::is_floating_point_v<T>, "neg_inf<T> requires floating-point type");
  return -std::numeric_limits<T>::infinity();
}
...
template <typename T>
constexpr T epsilon() {
  static_assert(std::is_floating_point_v<T>, "epsilon<T> requires floating-point type");
...
T safe_div(T a, T b) {
  if constexpr (std::is_floating_point_v<T>) {
    if (b == T(0)) {
      return (a == T(0)) ? nan<T>() : (a > T(0) ? inf<T>() : neg_inf<T>());
    }
    return a / b;
  } else {
    if (b == T(0)) {
...
T safe_log(T value) {
  ...
  if (value < T(0)) return nan<T>();
  if (value == T(0)) return neg_inf<T>();
