...
class ShapeError : public AxiomError {
 public:
  ...
  static ShapeError mismatch(const Container& expected, const Container& got) {
  ...
  static ShapeError invalid_axis(int axis, int ndim) {
    return ShapeError("axis " + std::to_string(axis) + " out of bounds for tensor with " +
                      std::to_string(ndim) + " dimensions");
  }

  static ShapeError invalid_reshape(size_t from_size, size_t to_size) {
    return ShapeError("cannot reshape tensor of size " + std::to_string(from_size) +
                      " to size " + std::to_string(to_size));
  }
};
...
class TypeError : public AxiomError {
 public:
  ...
  static TypeError unsupported_dtype(const std::string& dtype, const std::string& operation) {
    return TypeError("unsupported dtype '" + dtype + "' for " + operation);
  }

  static TypeError dtype_mismatch(const std::string& expected, const std::string& got) {
    return TypeError("expected dtype " + expected + " but got " + got);
  }

  static TypeError conversion_not_safe(const std::string& from, const std::string& to) {
    return TypeError("conversion from " + from + " to " + to + " may lose precision");
  }
};
...
class DeviceError : public AxiomError {
 public:
  ...
  static DeviceError mismatch(const std::string& expected, const std::string& got) {
    return DeviceError("expected tensor on " + expected + " but got " + got);
  }
...
class ValueError : public AxiomError {
 public:
  ...
  static ValueError nan_detected(const std::string& context = "") {
    return ValueError("NaN detected" + (context.empty() ? "" : " in " + context));
  }

  static ValueError inf_detected(const std::string& context = "") {
    return ValueError("Inf detected" + (context.empty() ? "" : " in " + context));
  }

  static ValueError not_finite(const std::string& context = "") {
    return ValueError("non-finite value detected" + (context.empty() ? "" : " in " + context));
  }

  static ValueError out_of_range(const std::string& what, double min, double max, double got) {
    std::ostringstream oss;
    oss << what << " must be in range [" << min << ", " << max << "] but got " << got;
...
class IndexError : public AxiomError {
 public:
  ...
  static IndexError out_of_bounds(size_t index, size_t size, int dim = -1) {
    ...
    if (dim >= 0) oss << "dimension " << dim << " with ";
    oss << "size " << size;
...
class MemoryError : public AxiomError {
 public:
  ...
  static MemoryError allocation_failed(size_t bytes) {
    return MemoryError("failed to allocate " + std::to_string(bytes) + " bytes");
  }
  ...
  static MemoryError storage_too_small(size_t required, size_t available) {
    return MemoryError("storage has " + std::to_string(available) +
                       " bytes but " + std::to_string(required) + " required");
  }
